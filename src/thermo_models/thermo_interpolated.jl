@muladd begin
    ...
    struct ThermoData1T{I<:Interpolation, CvO<:CvTableOffset, NCOMP} <: ThermoData{I, CvO, NCOMP}
        ...
        # N_cv_discretization is either N_T_discretization or N_cv_discretization+1 (if offset is used)
        ...
        function ThermoData1T{LinearInterpolation, NoCvOffset, NCOMP}(ref_q, mass_arr, e_c_int_function_arr;
                ...
                e_min_arr = minimum(e_arr, dims=1) ./ ref_q.e_ref
        ...
        function ThermoData1T(ref_q::ReferenceFlowQuantities, mass_arr, e_c_int_function_arr;
            T_min=10.0, T_max=3.0e4, ΔT=1.0, interpolation=:linear, cv_table_offset=false)
            NCOMP = length(mass_arr)
            if interpolation == :linear
                if cv_table_offset == true
                    error("Offset of c_v(T) w.r.t. e(T) not implemented")
                else
                    return ThermoData1T{LinearInterpolation, NoCvOffset, NCOMP}(ref_q, mass_arr, e_c_int_function_arr; T_min=T_min, T_max=T_max, ΔT=ΔT)
                end
            else
    ...
    @inline function energy_from_rho_vec(rho_vec::SVector, rho, T, thermodata::ThermoData1T{LinearInterpolation, NoCvOffset, NCOMP}) where {NCOMP}
        result = 0.0
    ...
    @inline function c_v_component(i_comp, index_lower_c, fracpos_c, thermodata::ThermoData1T{LinearInterpolation, CvO, NCOMP}) where {CvO, NCOMP}
        @inbounds return thermodata.c_v_arr[index_lower_c, i_comp] * (1.0 - fracpos_c) + fracpos * thermodata.c_v_arr[index_lower_c + 1, i_comp]
    end
    ...
    @inline function entropy_c_v_integral_component(i_comp, index_lower_c, fracpos_c, T_b, thermodata::ThermoData{LinearInterpolation, CvO, NCOMP}) where {CvO, NCOMP}
        @inbounds T_a = thermodata.T_arr[index_lower_c]
        @inbounds T_a_inv = thermodata.T_arr_inv[index_lower_c]
    ...
    @inline function temperature_rhoinv_with_index(u, rhoinv, T0, e, thermodata::ThermoData1T)
        ...
        if (T < thermodata.T_min_E)     # again, T_min_E or T_min_c_v?
